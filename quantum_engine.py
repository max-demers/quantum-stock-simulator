...
def initialize_parameters(N, action, resistance_prices=None):
    ...
    test_size = 100
    df_train = df.iloc[:-test_size] # On prend les données d'entrainement
    df_test = df.iloc[-test_size:] # On prend les données de test
    
    prices_train = df_train['Close'].values.flatten() # On prend les prix d'entrainement
    initial_price = prices_train[-1] # Le prix à l'instant T=0 de la simulation
    
    log_return = np.diff(np.log(prices_train)) # On calcule les retours log
    v_0 = np.std(log_return) # On calcule la volatilité
    ...
    if resistance_prices: # Si on a des résistances, on définit les bornes de l'espace des prix
        ...
        min_res = np.log(min(resistance_prices) * 0.95) # On prend le log de la résistance minimale
        x_min = min(x_min, min_res) # On définit la borne inférieure de l'espace des prix
        x_max = max(x_max, max_res) # On définit la borne supérieure de l'espace des prix

    x = np.linspace(x_min, x_max, N) # On crée un espace de prix
    dx = x[1] - x[0] # On calcule le pas de l'espace des prix
    mass = 1 / v_0 ** 2 # On calcule la masse
    
    # On retourne aussi les prix réels futurs pour la comparaison
    real_prices_future = df_test['Close'].values.flatten() # On prend les prix réels futurs
    real_times_future = df_test.index # On prend les dates réelles futures
...
def create_initial_wave_packet(x, x_0, v_0, k_0):
    ...
    gaussian = np.exp(-((x - x_0) ** 2) / (4 * v_0 ** 2)) # On crée une gaussienne
    phase = np.exp(1j * (k_0-(v_0**2/2)) * x) # On crée une phase
    A = (1 / (2 * np.pi * v_0 ** 2)) ** (1 / 4) # On normalise
...
def run_simulation_and_animate(psi, K_coeff, dt, S, x, potential_vector, steps_per_frame=10,
    ...
    for i in range(S): # On parcourt les pas de temps
        ...
        if i % steps_per_frame == 0: # Si on atteint un pas de temps pour l'affichage
            ...
            plt.pause(0.001) # On met à jour le graphique
